 ...
 *
 * @example
 *   npx tsx read-local-memory.ts -d ./aeon-memory示例数据
 *   npx tsx read-local-memory.ts -d ./aeon-memory示例数据 -L L0 --since 7d
 *   npx tsx read-local-memory.ts -d ./aeon-memory示例数据 -L L1 -f 'type=persona'
 */
...
Options:
  -d, --data-dir <路径>    本地 aeon-memory 数据目录路径（必填，须含 vectors.db）
...
Examples:
  ...
  # L0 分页：第 2 页（每页 20 条）
  npx tsx read-local-memory.ts -d ./aeon-memory示例数据 -L L0 -l 20 --offset 20

  # 以 JSON 格式输出
  npx tsx read-local-memory.ts -d ./aeon-memory示例数据 -L L0 --since 7d --format json
`.trim()
