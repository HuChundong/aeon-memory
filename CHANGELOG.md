...
- **关闭推理模型 thinking（`disableThinking`）**：新增 `llm.disableThinking` 与 `offload.disableThinking` 两项配置（均默认 `false`，不改变现有行为），支持多种推理引擎/模型提供商的关闭方式。
  ...
  - 环境变量 `AEON_MEMORY_LLM_DISABLE_THINKING` 支持策略名（如 `deepseek`）和布尔值。
...
- **Gateway 可选 Bearer 鉴权 + CORS 白名单**：新增 `server.apiKey` / `AEON_MEMORY_GATEWAY_API_KEY` 配置项，设置后所有非 `/health` 路由需携带 `Authorization: Bearer <key>`（`crypto.timingSafeEqual` 防时序攻击）；新增 `server.corsOrigins` / `AEON_MEMORY_CORS_ORIGINS` 配置项，显式指定允许的 CORS 来源列表（空列表 = 不发送 CORS 头，`"*"` = 保留旧版宽松行为）。启动时打印安全态势摘要，非回环地址 + 无 apiKey 时输出 WARN。两项均默认关闭，现有部署无需改动。Hermes Python 客户端同步支持 `MEMORY_TENCENTDB_GATEWAY_API_KEY` 环境变量自动附加 Bearer 头。
...
- **Docker 一体化镜像**（`Dockerfile.hermes`）：单容器捆绑 Hermes Agent + memory_tencentdb 插件 + Aeon Memory，统一 `MODEL_*` 环境变量驱动。
...
- **声明 `contracts.tools`**：注册 `aeon-memory_memory_search`、`aeon-memory_conversation_search` 工具名，满足 tool registration contract 要求。
...
- 重构为 `AeonMemoryCore` 宿主无关的核心层 + 适配器模式，解耦 OpenClaw 框架依赖
...
- `AeonMemoryCore` 提供统一的 `handleBeforeRecall()` / `handleTurnCommitted()` / `searchMemories()` 等 API
...
- `aeon-memory_memory_search` + `aeon-memory_conversation_search` 增加每轮合计最多 3 次的调用次数限制，通过 tool description 和召回引导提示词约束模型行为，防止陷入无效重复搜索
...
- *(aeon-memory)* 用 reporter 抽象替换 emitMetric
